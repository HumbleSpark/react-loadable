...
function load(loader) {
  ...
  if (reported.length > 1) {
    throw new Error(
      'react-loadable cannot handle more than one import() in each loader',
    );
  }
  ...
  try {
    if (isWebpackBundle) {
      if (typeof state.metadata.webpackRequireWeakId === 'function') {
        state.loaded = webpackRequireWeak(
          state.metadata.webpackRequireWeakId(),
        );
  ...
  return class LoadableComponent extends React.Component {
    ...
    render() {
      if (typeof window === 'undefined' && res.metadata) {
        flushedMetadata.push(res.metadata);
      }
...
function flush() {
  let _flushedMetadata = flushedMetadata;
  flushedMetadata = [];
  return _flushedMetadata;
}
